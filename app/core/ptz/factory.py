...
class PTZControllerFactory:
    ...
    def register(cls, ptz_type: str):
        ...
        def decorator(controller_cls: Type["BasePTZController"]):
            ...
            logger.info(f"Registered PTZ controller: {ptz_type} -> {controller_cls.__name__}")
            return controller_cls
        return decorator

    @classmethod
    def create(cls, ptz_type: str, config: CameraConfig) -> Optional["BasePTZController"]:
        """
        ...
        if controller_cls is None:
            logger.error(f"Unknown PTZ type: {ptz_type}. Available: {list(cls._registry.keys())}")
            return None

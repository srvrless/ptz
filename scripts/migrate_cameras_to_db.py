...
def migrate_cameras_from_env_to_db() -> None:
    ...
    if not cameras_from_env:
        logger.warning(
            "No cameras found in .env (check CAMERAS variable and camera1_*, camera2_*, etc.)"
        )
        return
    ...
    try:
        ...
        for camera_id, cam_cfg in cameras_from_env.items():
            ...
            ptz_type = (
                db_session.query(PTZType)
                .filter(PTZType.type == cam_cfg.ptz_type.lower())
                .first()
            )
            if not ptz_type:
                logger.warning(
                    f"PTZ type '{cam_cfg.ptz_type}' not found, using 'onvif'"
                )
                ptz_type = onvif_type
        ...
        db_session.commit()
        logger.info(
            f"Migration complete! {len(cameras_from_env)} cameras migrated to DB"
        )
